...
hold on
grid on
figure_name = "fmincon scan: 4 piecewise best-fit equation";
title(figure_name)
xlabel("X Data")
ylabel("Y Data")
...
% Equation 4: y3 = a4*x^2 + b4*x + c4
% Breakpoints: min(x_data) < optimal_x1 < optimal_x2 < optimal_x3 < max(x_data)
...
COEFFICIENT_VAL_MIN = -10e10;
COEFFICIENT_VAL_MAX = 10e10;
...
scan_step = (max(x_data) - min(x_data)) / 50;
for optimal_x1 = min(x_data) : scan_step : max(x_data)
    for optimal_x2 = optimal_x1 : scan_step : max(x_data)
        for optimal_x3 = optimal_x2 : scan_step : max(x_data)
            % Define the objective function
            optimal_breakpoints = [optimal_x1, optimal_x2, optimal_x3];
            ...
            if cost_func_result < min_cost_func_result
                ...
                optimal_x3_final = optimal_x3;
        
                fprintf('%d) Cost function = %.10f, optimal_x1 = %.10f, optimal_x2 = %.10f, optimal_x3 = %.10f\n', update_optimal_counter, min_cost_func_result, optimal_x1_final, optimal_x2_final);
            end
        end
    end
end
...
b3 = optimal_params(8);
c3 = optimal_params(9);
a4 = optimal_params(10);
b4 = optimal_params(11);
c4 = optimal_params(12);
optimal_breakpoints = [optimal_x1_final, optimal_x2_final, optimal_x3_final];
...
plot(x4, y4, 'm', 'DisplayName', 'Piecewise Fit: y4', 'LineWidth', 2);
legend show
...
annotation_text = sprintf(['Optimal solution:\n', ...
                           ...
                           'Equation 4: y4 = %.10f*x^2 + %.10f*x + %.10f\n', ...
                           'Breakpoint 1 = %.10f\n', ...
                           'Breakpoint 2 = %.10f', ...
                           'Breakpoint 3 = %.10f'], ...
                           objective_function(optimal_params, optimal_breakpoints, x_data, y_data), ...
                           ...
                           a3, b3, c3, ...
                           a4, b4, c4, ...
                           optimal_x1_final, optimal_x2_final, optimal_x3_final);
...
saveas(gcf, 'test_fmincon_scan.png');
...
function [c, ceq] = continuity_constraints(params, optimal_breakpoints)
    ...
    y3_at_x3 = a3*optimal_breakpoints(3)^2 + b3*optimal_breakpoints(3) + c3;
    y4_at_x3 = a4*optimal_breakpoints(3)^2 + b4*optimal_breakpoints(3) + c4;
    ...
    y3_gradient_at_x3 = 2 * a3 * optimal_breakpoints(3) + b3;
    y4_gradient_at_x3 = 2 * a4 * optimal_breakpoints(3) + b4;
    ...
    ceq = [y1_at_x1 - y2_at_x1;
           ...
           y3_gradient_at_x3 - y4_gradient_at_x3;
    ];
